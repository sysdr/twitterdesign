...
import { createEngine, DIMS, runQuantBenchmark, resetDashboardMetrics } from './engine.mjs';

const PORT = parseInt(process.env.PORT ?? '3190', 10);
...
await runQuantBenchmark(engine);
console.log(`[server] seeded ${SEED_TOPICS.length} tweets + SQ/PQ bench  http://localhost:${PORT}`);
...
function statsPayload() {
  ...
  return {
    ...
    compressionRatioSQ: s.compressionRatioSQ,
    compressionRatioPQ: s.compressionRatioPQ,
    recallSQat10: s.recallSQat10,
    recallPQat10: s.recallPQat10,
    floatSearchMs: s.floatSearchMs,
    sqSearchMs: s.sqSearchMs,
    pqSearchMs: s.pqSearchMs,
    floatBytes: s.floatBytes,
    sqBytes: s.sqBytes,
    pqBytes: s.pqBytes,
    codebookBytes: s.codebookBytes,
    lastBenchAt: s.lastBenchAt,
...
<title>NEXUS Day 19 - Vector quantization (SQ / PQ)</title>
...
<style>
  :root{
    --bg:#eaf6ff;
    --surface:rgba(245,252,255,.94);
    --surface2:#f4fbff;
    --line:rgba(14,116,144,.20);
    --line2:rgba(2,132,199,.22);
    --text:#0f3b5f;
    --muted:#0b5e8c;
    --accent:#0284c7;
    --mint:#0369a1;
    --shadow:0 20px 50px rgba(3,105,161,.14);
    --radius:16px;
  }
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .ambient{
    ...
    background:
      radial-gradient(ellipse 90% 55% at 50% -15%,rgba(125,211,252,.38),transparent 55%),
      radial-gradient(ellipse 50% 45% at 100% 30%,rgba(186,230,253,.40),transparent 50%),
      radial-gradient(ellipse 45% 50% at 0% 85%,rgba(219,244,255,.65),transparent 45%),
      linear-gradient(180deg,#f0f9ff 0%,var(--bg) 42%,#dbeafe 100%);
  }
  .ambient::after{
    content:'';position:absolute;inset:0;opacity:.06;
    background-image:linear-gradient(rgba(3,105,161,.12) 1px,transparent 1px),
      linear-gradient(90deg,rgba(3,105,161,.12) 1px,transparent 1px);
  ...
  .top{
    ...
    background:linear-gradient(145deg,rgba(250,245,255,.96) 0%,rgba(237,233,254,.9) 100%);
  ...
  .toolbar{
    ...
    max-width:740px;justify-content:flex-end;
  ...
  .btn{
    ...
    transition:transform .12s ease,filter .12s ease,box-shadow .14s ease;
  }
  .btn:hover{filter:brightness(1.07);box-shadow:0 8px 18px rgba(91,33,182,.26);transform:translateY(-1px)}
  ...
  .card:hover{box-shadow:0 18px 40px rgba(91,33,182,.16),inset 0 1px 0 rgba(255,255,255,.95)}
  .card::before{
  ...
  .stat{
    ...
    padding:10px 12px;margin-bottom:7px;font-size:.76rem;
    border-radius:10px;
    background:linear-gradient(120deg,rgba(237,233,254,.72),rgba(243,236,255,.7));
    border:1px solid rgba(167,139,250,.35);
  }
  .stat:nth-child(even){background:rgba(221,214,254,.45)}
  .v{color:var(--mint);font-weight:600}
  .search-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:10px}
  ...
  .inp-row{display:flex;gap:10px;align-items:stretch;flex-wrap:wrap;margin-bottom:12px}
  .inp-row input{
  ...
  .layout-left,.layout-right{display:grid;gap:14px}
...
<header class="top">
  <div class="brand">
    <h1>NEXUS Day 19 — SQ / PQ quantization</h1>
    <p class="sub">Scalar & product quantization · recall@10 vs float · compression ratios</p>
  </div>
  <div class="toolbar">
    <span class="engine-pill" title="Stats + demo refresh every 2s while tab visible">● Live 2s</span>
...
<div class="grid">
  ...
  <div class="card">
    <h2>Embed benchmark (50 iters × 100 tweets)</h2>
    <div id="bench"><div class="stat"><span>click Run Benchmark</span></div></div>
    <button type="button" class="btn btn-cyan" onclick="runBench()">▶ Run Embed Benchmark</button>
  </div>
</div>
<div class="grid">
  <div class="card">
    <h2>Quantization — memory</h2>
    <div id="quantMem"><div class="stat"><span>loading…</span></div></div>
  </div>
  <div class="card">
    <h2>Quantization — search &amp; recall@10</h2>
    <div id="quantSearch"><div class="stat"><span>loading…</span></div></div>
    <button type="button" class="btn btn-cyan" onclick="runQuantBench()">▶ Re-run SQ/PQ bench</button>
  </div>
</div>
...
async function loadStats(){
  ...
  document.getElementById('stats').innerHTML=
    '<div class="stat"><span>node</span><span class="v">'+escapeHtml(String(s.node))+'</span></div>'+
    ...
    '<div class="stat"><span>poll time (server)</span><span class="v">'+escapeHtml((s.serverTime||'').slice(11,23))+'</span></div>';
  ...
  var cr=function(n){return (n!==undefined&&n!==null&&n!=='')?n:'—';};
  document.getElementById('quantMem').innerHTML=
    '<div class="stat"><span>float bytes (index)</span><span class="v">'+cr(s.floatBytes)+'</span></div>'+
    '<div class="stat"><span>SQ bytes (384×uint8)</span><span class="v">'+cr(s.sqBytes)+'</span></div>'+
    '<div class="stat"><span>PQ codes + codebook</span><span class="v">'+cr(s.pqBytes)+' + '+cr(s.codebookBytes)+'</span></div>'+
    '<div class="stat"><span>compression SQ</span><span class="v">'+cr(s.compressionRatioSQ)+'×</span></div>'+
    '<div class="stat"><span>compression PQ</span><span class="v">'+cr(s.compressionRatioPQ)+'×</span></div>';
  document.getElementById('quantSearch').innerHTML=
    '<div class="stat"><span>float topK ms</span><span class="v">'+cr(s.floatSearchMs)+'</span></div>'+
    '<div class="stat"><span>SQ topK ms</span><span class="v">'+cr(s.sqSearchMs)+'</span></div>'+
    '<div class="stat"><span>PQ topK ms</span><span class="v">'+cr(s.pqSearchMs)+'</span></div>'+
    '<div class="stat"><span>recall@10 SQ vs float</span><span class="v">'+cr(s.recallSQat10)+'</span></div>'+
    '<div class="stat"><span>recall@10 PQ vs float</span><span class="v">'+cr(s.recallPQat10)+'</span></div>';
  }catch(err){
    var msg=err&&err.message?err.message:String(err);
    document.getElementById('stats').innerHTML='<div class="stat"><span>stats</span><span class="v">'+escapeHtml(msg)+'</span></div>';
  }
}
async function runQuantBench(){
  document.getElementById('quantSearch').innerHTML='<div class="stat"><span>running…</span></div>';
  try{
    var rq=await fetch('/api/quant-bench');
    if(!rq.ok) throw new Error('quant-bench '+rq.status);
    await rq.json();
    await loadStats();
  }catch(e){
    document.getElementById('quantSearch').innerHTML='<div class="stat"><span>error</span><span class="v">'+escapeHtml(e.message)+'</span></div>';
  }
}
async function runBench(){
  document.getElementById('bench').innerHTML='<div class="stat"><span>running…</span></div>';
  try{
    var rb=await fetch('/api/benchmark');
    if(!rb.ok) throw new Error('benchmark '+rb.status);
    var b=await rb.json();
    ...
    document.getElementById('bench').innerHTML='<div class="stat"><span>error</span><span class="v">'+escapeHtml(e.message)+'</span></div>';
  }
}
function searchK(){
...
async function exportIndex(){
  ...
  a.download='nexus-day19-index.json';
  a.click();
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/quant-bench' && req.method === 'GET') {
      engine.rebuildQuantIndex();
    ...
    if (path === '/api/export' && req.method === 'GET') {
      ...
      res.writeHead(200, {
        ...
        'Content-Disposition': 'attachment; filename="nexus-day19-index.json"',
      });
