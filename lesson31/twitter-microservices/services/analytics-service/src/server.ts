...
class AnalyticsService {
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/', (req, res) => {
      res.json({ message: 'analytics-service is running', timestamp: new Date() });
    });
  }
  ...
  public start(): void {
    this.app.listen(this.port, () => {
      console.log(`⚙️  analytics-service running on port ${this.port}`);
