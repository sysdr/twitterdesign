...
exports.FailurePredictor = void 0;
class FailurePredictor {
    // Calculate survival probability using fitted distribution
    calculateSurvivalProbability(distribution, currentAge, futureTime) {
    ...
    predict(metrics, analysis) {
        ...
        const survival1Hour = this.calculateSurvivalProbability(analysis.distribution, currentAge, 1);
        const survival6Hour = this.calculateSurvivalProbability(analysis.distribution, currentAge, 6);
        const survival24Hour = this.calculateSurvivalProbability(analysis.distribution, currentAge, 24);
        ...
        const adjustedProb1Hour = this.adjustProbabilityWithCurrentState(probability1Hour, lastMetric);
        // Estimate time to failure (when probability reaches 50%)
        const timeToFailure = this.estimateTimeToFailure(analysis.distribution, currentAge);
        // Determine risk level
        const riskLevel = this.determineRiskLevel(adjustedProb1Hour, lastMetric);
        // Generate recommendations
        const recommendations = this.generateRecommendations(riskLevel, lastMetric, analysis);
        return {
    ...
    adjustProbabilityWithCurrentState(baseProbability, metric) {
        let adjustment = 1.0;
    ...
    determineRiskLevel(probability, metric) {
    ...
    generateRecommendations(riskLevel, metric, analysis) {
...
exports.FailurePredictor = FailurePredictor;
